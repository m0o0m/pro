...
class Connection extends Component {
    ...
    public function open() {
        ...
        $this->_socket = @stream_socket_client(
                        $this->unixSocket ? 'unix://' . $this->unixSocket : 'tcp://' . $this->hostname . ':' . $this->port, $errorNumber, $errorDescription, $this->connectionTimeout ? $this->connectionTimeout : ini_get("default_socket_timeout"), $this->socketClientFlags
        );
        if ($this->_socket) {
    ...
    public function __call($name, $params) {
        ...
        if (in_array($redisCommand, $this->redisCommands)) {
            if ($redisCommand === 'GET') {
                ...
                $res = $this->executeCommand($name, $params);
                if ($res === NULL) {
                    return false;
                }
                if ($compress) {
                ...
                if ($compress) {
                    $params[1] = gzcompress($params[1]);
                }
            } else if ($redisCommand === 'SETEX') {
                $compress = true;
                if (isset($params[3])) {
                    $compress = $params[3];
                    unset($params[3]);
                ...
                if ($compress) {
                    $params[2] = gzcompress($params[2]);
                }
            }

            if (strtoupper($name) == 'HGETALL') {
                $data = $this->executeCommand($name, $params);
                if (!empty($data)) {
                    $return = [];
                    $keys = [];
                    $values = [];
    ...
    public function executeCommand($name, $params = []) {
        ...
        if (strtoupper($name) == 'HMSET' || strtoupper($name) == 'MGET') {
            $a = [];
