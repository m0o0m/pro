    ...
    <div class="pk_bj">
        <div class="footer1">
            ...
            <div class="one"><span class="left">金额￥</span>
                <I-Input :maxlength="9" style="width: 100px" size="small" v-model="money" :value="money"
                         @on-blur="onblur_top(0)" @on-focus="onfocus_top(0)" @on-change="change_money()"
        ...
        <div class="pk_bj_box">
          ...
          <em>{{key.odd}}</em>
            ...
            <I-Input @on-keydown="tab_now(key)" @on-change="onchange(key)" :value="key.money" @on-blur="onblur(key)"
                     :maxlength="9" ref="myfocus" size="small" style="width: 45px;" v-model="key.money"
                     @on-focus="onfocus(key)" @on-keyup="gogo(key)" @on-afterpaste="gogo(key)"></I-Input>
          </span>
                </li>
            </ul>
        </div>
        <div class="footer1" style="margin-bottom: 20px">
            ...
            <div class="one"><span class="left">金额￥</span>
                <I-Input :maxlength="9" style="width: 100px" size="small" v-model="money" :value="money"
                         @on-focus="onfocus_top(1)" @on-blur="onblur_top(1)" @on-change="change_money()"
                         @on-keyup="push_money()" @on-afterpaste="push_money()"></I-Input>
            </div>
            <button type="button" class="two" style="padding: 8px;" @click="go_to()">立即下注</button>
            <button type="button" class="two" style="padding: 8px;margin-right:20px;" @click="reset()">重置</button>
        ...
        methods: {
            ...
            computed_money: function () {
                for (let i = 0; i < this.onelist.length; i++) {
                    for (let j = 0; j < this.onelist[i].object.length; j++) {
                        //添加金额参数入对象
                        if (this.onelist[i].object[j].flag) {
                            this.onelist[i].object[j].money = this.money
                        } else if (!this.onelist[i].object[j].flag) {
                            this.onelist[i].object[j].money = ''
                        }
                    }
                }
            },
            ...
            go_to() {
                ...
                for (let i = 0; i < this.onelist.length; i++) {
                    for (let j = 0; j < this.onelist[i].object.length; j++) {
                        let b = this.onelist[i].object[j].money + 'b';
                        this.onelist[i].object[j].money = b.replace(/\D/g, "");
                        kk += Number(this.onelist[i].object[j].money);
                        if (this.onelist[i].object[j].flag) {
                            is_select = true
                        }
                    }
...
//            document.querySelector('body').style.overflow = 'hidden';
                        this.$root.$emit('id-selected', this.onelist);
...
<style lang="scss" src="../../../assets/css/pk_bj.scss" scoped>
</style>
