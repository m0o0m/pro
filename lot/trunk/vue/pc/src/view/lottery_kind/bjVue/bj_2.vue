<template>
  <div class="pk_bj">
    <div class="footer1">
      ...
      <div class="one"><span class="left">金额￥</span><I-Input :maxlength="9" style="width: 100px" size="small" v-model="money" :value="money" @on-blur="onblur_top(0)"  @on-focus="onfocus_top(0)" @on-change="change_money()"  @on-keyup="push_money()" @on-afterpaste="push_money()"></I-Input>
      </div>
      ...
      <ul v-for="item in towlist">
        ...
        <li v-for="(key,index) in item.object" :class="[key.flag?'styleclect':'']">
          ...
          <span class="three" @click.self="myclick(key,index)">
            <I-Input @on-keydown="tab_now(key)" @on-change="onchange(key)" ref="myfocus" size="small" style="width: 45px;" :maxlength="9" v-model="key.money" :value="key.money" @on-focus="onfocus(key)" @on-keyup="gogo(key)" @on-blur="onblur(key)" @on-afterpaste="gogo(key)" ></I-Input>
          </span>
        </li>
      </ul>
    </div>
    <div class="footer1" style="margin-bottom: 20px">
      ...
      <div class="one"><span class="left">金额￥</span><I-Input style="width: 100px" size="small" :maxlength="9" v-model="money" :value="money" @on-focus="onfocus_top(1)"  @on-blur="onblur_top(1)" @on-change="change_money()" @on-keyup="push_money()" @on-afterpaste="push_money()"></I-Input>
      </div>
      <button type="button" class="two" style="padding: 8px;" @click="go_to()">立即下注</button>
      <button type="button" class="two" style="padding: 8px;margin-right:20px;" @click="reset()">重置</button>
    </div>
