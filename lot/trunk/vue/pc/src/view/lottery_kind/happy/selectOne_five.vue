<template>
  <div class="selectone_five">
      ...
      <ul v-for="item in one">
        ...
        <li v-for="(key,index) in item.object" :class="[key.flag?'styleclect':'']">
          ...
          <span v-if="key.num" class="three" @click.self="numClick(key,index)">
            <I-Input @on-keydown="tab_now(key)" @on-change="onchange(key)" :value="key.money" @on-blur="onblur(key)" :maxlength="9" ref="myfocus" v-model="key.money" @on-focus="onfocus(key)" @on-keyup="gogo(key)" @on-afterpaste="gogo(key)" size="small" style="width: 60px;"></I-Input>
          </span>
        </li>
      </ul>
    </div>
    <div class="footer1" style="margin-bottom: 20px">
      ...
      <div class="one"><span class="left">金额￥</span><I-Input :value="money" @on-blur="onblur_top(1)"  @on-focus="onfocus_top(1)" @on-change="change_money()" :maxlength="9" style="width: 100px" size="small" v-model="money"  @on-keyup="push_money()" @on-afterpaste="push_money()"></I-Input>
      </div>
