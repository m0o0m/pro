        ...
        <li v-for="(key,k) in item.object" class="cen" :class="{'table-current':key.flag}">
          <ul class='clearfix'>
            ...
            <li class="three fl" @click.self="numClick(key)">
                <I-Input :value="key.money" @on-blur="onblur(key)" @on-keydown="tab_now(key)" @on-change="onchange(key)" :maxlength="9" ref="myfocus" style="width: 75px" @on-focus="onfocus(key)" v-model="key.money" size="small" @on-keyup="gogo(key)" @on-afterpaste="gogo(key)"></I-Input>
            </li>
          </ul>
        </li>
      </ul>
