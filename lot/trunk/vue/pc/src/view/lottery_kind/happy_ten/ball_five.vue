      ...
      <ul v-for="item in fivelist">
        ...
        <li v-for="(key,index) in item.object" :class="[key.flag?'styleclect':'']">
          ...
          <span class="three" @click.self="myclick(key,index)">
             <I-Input @on-keydown="tab_now(key)" :value="key.money" @on-blur="onblur(key)" @on-change="onchange(key)" :maxlength="9" v-if="key.num" ref="myfocus" size="small" style="width: 60px;" @on-focus="onfocus(key)" v-model="key.money"
  ...
  export default {
    ...
    methods: {
      ...
      go_to() {
        ...
        var kk = 0;
        var is_select = false;
        for (let i = 0; i < this.fivelist.length; i++) {
          for (let j = 0; j < this.fivelist[i].object.length; j++) {
            if (this.fivelist[i].object[j].money) {
              let b = this.fivelist[i].object[j].money + 'b';
              this.fivelist[i].object[j].money = b.replace(/\D/g, "");
              kk += Number(this.fivelist[i].object[j].money);
              if(this.fivelist[i].object[j].flag){
                is_select = true
              }
            }
          }
...
//            document.querySelector('body').style.overflow = 'hidden';
            this.$root.$emit('id-selected', this.fivelist);
      ...
      reset: function () {
        for (let i = 0; i < this.fivelist.length; i++) {
          for (let j = 0; j < this.fivelist[i].object.length; j++) {
            this.fivelist[i].object[j].money = '';
            this.fivelist[i].object[j].flag = '';
          }
        }
        this.money = '';
      ...
      computed_money: function () {
        for (let i = 0; i < this.fivelist.length; i++) {
          for (let j = 0; j < this.fivelist[i].object.length; j++) {
            //添加金额参数入对象
            if (this.fivelist[i].object[j].flag) {
              this.fivelist[i].object[j].money = this.money
            } else if (!this.fivelist[i].object[j].flag) {
              this.fivelist[i].object[j].money = ''
            }
          }
        }
