      ...
      <ul v-for="item in fourlist">
        ...
        <li v-for="(key,index) in item.object" :class="[key.flag?'styleclect':'']">
          ...
          <span class="three" @click.self="myclick(key,index)">
             <I-Input @on-keydown="tab_now(key)" @on-change="onchange(key)" :value="key.money" @on-blur="onblur(key)" :maxlength="9" v-if="key.num" ref="myfocus" @on-focus="onfocus(key)" size="small" style="width: 60px;" v-model="key.money"
  ...
  export default {
    ...
    methods: {
      ...
      go_to() {
        ...
        var kk = 0;
        var is_select = false;
        for (let i = 0; i < this.fourlist.length; i++) {
          for (let j = 0; j < this.fourlist[i].object.length; j++) {
            if (this.fourlist[i].object[j].money != undefined) {
              let b = this.fourlist[i].object[j].money + 'b';
              this.fourlist[i].object[j].money = b.replace(/\D/g, "");
              kk += Number(this.fourlist[i].object[j].money);
              if(this.fourlist[i].object[j].flag){
                is_select = true
              }
            }
          }
...
//            document.querySelector('body').style.overflow = 'hidden';
            this.$root.$emit('id-selected', this.fourlist);
      ...
      reset: function () {
        for (let i = 0; i < this.fourlist.length; i++) {
          for (let j = 0; j < this.fourlist[i].object.length; j++) {
            this.fourlist[i].object[j].money = '';
            this.fourlist[i].object[j].flag = '';
          }
        }
        this.money = '';
      ...
      computed_money: function () {
        for (let i = 0; i < this.fourlist.length; i++) {
          for (let j = 0; j < this.fourlist[i].object.length; j++) {
            //添加金额参数入对象
            if (this.fourlist[i].object[j].flag) {
              this.fourlist[i].object[j].money = this.money
            } else if (!this.fourlist[i].object[j].flag) {
              this.fourlist[i].object[j].money = ''
            }
          }
        }
