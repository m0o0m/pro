      ...
      <ul v-for="item in twolist">
        ...
        <li v-for="(key,index) in item.object" :class="[key.flag?'styleclect':'']">
          ...
          <span class="three" @click.self="myclick(key,index)">
             <I-Input @on-keydown="tab_now(key)" @on-change="onchange(key)" :maxlength="9" class="inp" ref="myfocus" style="width: 60px" @on-focus="onfocus(key)" :value="key.money" @on-blur="onblur(key)" v-model="key.money" size="small" @on-keyup="gogo(key)" @on-afterpaste="gogo(key)"></I-Input>
           </span>
        </li>
      </ul>
    </div>
    ...
    methods: {
      ...
      go_to() {
        ...
        for (let i = 0; i < this.twolist.length; i++) {
          for (let j = 0; j < this.twolist[i].object.length; j++) {
            if (this.twolist[i].object[j].money != undefined) {
              let b = this.twolist[i].object[j].money + 'b';
              this.twolist[i].object[j].money = b.replace(/\D/g, "");
              kk += Number(this.twolist[i].object[j].money);
              if(this.twolist[i].object[j].flag){
                is_select = true
              }
            }
          }
...
//            document.querySelector('body').style.overflow = 'hidden';
            this.$root.$emit('id-selected', this.twolist);
      ...
      reset: function () {
        for (let i = 0; i < this.twolist.length; i++) {
          for (let j = 0; j < this.twolist[i].object.length; j++) {
            this.twolist[i].object[j].money = '';
            this.twolist[i].object[j].flag = '';
          }
        }
        this.money = '';
      ...
      computed_money: function () {
        for (let i = 0; i < this.twolist.length; i++) {
          for (let j = 0; j < this.twolist[i].object.length; j++) {
            //添加金额参数入对象
            if (this.twolist[i].object[j].flag) {
              this.twolist[i].object[j].money = this.money
            } else if (!this.twolist[i].object[j].flag) {
              this.twolist[i].object[j].money = ''
            }
          }
        }
