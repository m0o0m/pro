<template>
  <div class="happy_ball">
      ...
      <ul v-for="item in threelist">
        ...
        <li v-for="(key,index) in item.object" :class="[key.flag?'styleclect':'']">
          ...
          <span class="three" @click.self="myclick(key,index)">
             <I-Input @on-keydown="tab_now(key)" :value="key.money" @on-blur="onblur(key)" @on-change="onchange(key)" :maxlength="9" v-if="key.num" ref="myfocus" size="small" style="width: 60px;" v-model="key.money"
                    @on-keyup="gogo(key)" @on-focus="onfocus(key)" @on-afterpaste="gogo(key)"></I-Input>
           </span>
        </li>
      </ul>
    </div>
    <div class="footer1" style="margin-bottom: 20px">
      ...
      <div class="one"><span class="left">金额￥</span><I-Input @on-change="change_money()" :value="money" @on-focus="onfocus_top(1)" @on-blur="onblur_top(1)" :maxlength="9" style="width: 100px" size="small"
    ...
    methods: {
      ...
      go_to() {
        ...
        for (let i = 0; i < this.threelist.length; i++) {
          for (let j = 0; j < this.threelist[i].object.length; j++) {
            if (this.threelist[i].object[j].money != undefined) {
              let b = this.threelist[i].object[j].money + 'b';
              this.threelist[i].object[j].money = b.replace(/\D/g, "");
              kk += Number(this.threelist[i].object[j].money);
              if(this.threelist[i].object[j].flag){
                is_select = true
              }
            }
          }
...
//            document.querySelector('body').style.overflow = 'hidden';
            this.$root.$emit('id-selected', this.threelist);
      ...
      reset: function () {
        ...
        for (let i = 0; i < this.threelist.length; i++) {
          for (let j = 0; j < this.threelist[i].object.length; j++) {
            this.threelist[i].object[j].money = '';
            this.threelist[i].object[j].flag = '';
          }
        }
        let dom = document.querySelectorAll('input');
        for(let i = 0;i < dom.length;i++){
      ...
      computed_money: function () {
        for (let i = 0; i < this.threelist.length; i++) {
          for (let j = 0; j < this.threelist[i].object.length; j++) {
            //添加金额参数入对象
            if (this.threelist[i].object[j].flag) {
              this.threelist[i].object[j].money = this.money
            } else if (!this.threelist[i].object[j].flag) {
              this.threelist[i].object[j].money = ''
            }
          }
        }
