...
$worker = new Worker('http://0.0.0.0:3336');
$worker->name = 'tenspider';
    ...
    Timer::add(10, function()use($typelist) {
        foreach ($typelist as $type) {
            ...
            foreach ($class::$wait as $qishu => $time) {
                ...
                foreach ($data as $val) {
                    if ($val['expect'] == $qishu) {
                        $res = Common_helper::addNewNumber($val, $type, 8);
                        if (!$res) {
