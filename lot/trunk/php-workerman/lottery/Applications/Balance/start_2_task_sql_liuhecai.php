...
$http_worker = new Worker("text://0.0.0.0:10005");

$http_worker->name = 'Mysql_For_Liuhecai';
   ...
   //更新胜的注单
    ...
    if($mycat){
        foreach($mycat_insert as $sql){
            if (!$redis->rpush('user_cash_sql' , $sql)) {
    ...
    if (!empty($mongo_update)) {
        ...
        foreach ($mongo_update as $val) {
            $redis->rpush('mongo_update', json_encode($val));
            // $mongo->updateData(
