<template>
  <div class="or">
    <div class="box">
      <div class="clearfix">
        <ul v-for="item in Olists" class="uls">
          <li class="thend">{{item.name}}</li>
          <li>
            <ul>
              <li v-for="key in item.object" :class="[key.flag?'styleclect':'']" class="list clearfix">
                <span class="one" @click="myclick(key)">{{key.name}}</span>
                <span class="tow" @click="myclick(key)">{{key.odd}}</span>
                <span class="three" @click.self="myclick(key)">
                  <I-Input :value="key.money" @on-blur="onblur(key)" @on-keydown="tab_now(key)" @on-change="onchange(key)" :maxlength="9" ref="myfocus" style="width: 55px" @on-focus="onfocus(key)" v-model="key.money" size="small" @on-keyup="gogo(key)" @on-afterpaste="gogo(key)"></I-Input>
                </span>
              </li>
            </ul>
          </li>
  ...
  methods: {
    myclick: function(item, i) {
        ...
        if(item.money && item.flag == true){
          item.flag = false;
          item.money = '';
          this.$refs.myfocus[item.or_id].blur();
          this.$refs.myfocus[item.or_id].$refs.input.value = '';
        }else if(item.money == '' && item.flag == false){
          this.$refs.myfocus[item.or_id].focus();
      ...
      onfocus: function(item){
        this.$refs.myfocus[item.or_id].$refs.input.data_onoff = 'true';
        this.a = item.or_id;
      ...
      onblur: function (key) {
          key.money = this.$refs.myfocus[key.or_id].$refs.input.value;
...
<style lang="scss" src="../../../assets/css/often_or.scss" scoped>
