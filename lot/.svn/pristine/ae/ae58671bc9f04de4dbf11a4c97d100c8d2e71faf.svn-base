...
class SetlimitModel extends \yii\base\Model {
    ...
    public static function sql($table, $sql) {
    ...
    public static function gameplay($request) {
        $fc_type = isset($request['fc_type']) ? trim($request['fc_type']) : null;

        if (empty($fc_type)) {
            return [];
        }
        return self::getItems(self::getTableName(), ['fc_type'=>$fc_type]);
    }
    ...
    public static function getItem($request) {
        ...
        if ($iscustom == 1) {
            $item = self::getOne(self::getTableName('agent'), ['id' => $id]);
        } elseif ($iscustom == -1) {
            $item = self::getOne(self::getTableName('line'), ['id' => $id]);
        } else {
            $item = self::getOne(self::getTableName(), ['id' => $id], '*', true);
        }
        return $item;
    }

    public static function getSetlimit($request) {
        $line_id = isset($request['line_id']) ? trim($request['line_id']) : null;
        $agent_type = isset($request['agent_type']) ? trim($request['agent_type']) : null;
        ...
        if ($agent_type && $agent_id) {
            ...
            $tmp = [];
            foreach ($agentCustom as $k => $v) {
                $v['iscustom'] = 1;
                $tmp[$v['fc_type'] . $v['gameplay']] = $v;
            }
            $agentCustom = $tmp;
        }
        $lineCustom = [];
