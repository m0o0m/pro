<template>
  <div class="happy_too">
    <div class="footer1">
      ...
      <div class="one"><span class="left">金额￥</span><I-Input :value="money" @on-blur="onblur_top(0)" @on-focus="onfocus_top(0)" @on-change="change_money()" :maxlength="9" style="width: 100px" size="small"
        ...
        <ul v-for="item in toplist" class="ul2">
          <li v-for="(key,index) in item.object" :class="[key.flag?'styleclect':'']">
            ...
            <span class="three" @click.self="myclick(key,index)">
              <I-Input @on-keydown="tab_now(key)" :value="key.money" @on-blur="onblur(key)" @on-change="onchange(key)" :maxlength="9" class="inp" ref="myfocus" style="width: 60px" @on-focus="onfocus(key)" v-model="key.money" size="small"
