<template lang="html">
  <div class="six_one">
    <nav-center v-show="this.$route.query.page == 'liuhecai'" :menus="lists" @go_child="go_child" :margin="false"></nav-center>
    <div v-show="this.$route.query.page != 'liuhecai'" style="width: 100%;margin-top: 15px"></div>
    ...
    <div class="content" v-show="showA">
      <div class="top">
        <ul>
          <li class="one">号码</li>
          <li class="two">赔率</li>
        ...
        <div class="bottom" v-for="item in lists1">
          <ul v-for="key in item.object" :class="[key.flag?'table-current':'']">
            ...
            <li @click.self="pour(key)" :class="[key.num?'three':'border3_none']">
              <I-Input :value="key.money" @on-blur="onblur(key,1)" @on-keydown="tab_now(key)" @on-change="onchange(key)" :maxlength='9' ref="input" @on-focus="onfocus(key,1)" style="width: 45px"  v-if="key.num" v-model="key.money" @on-keyup="gogo(key)" @on-afterpaste="gogo(key)" size="small"></I-Input>
            </li>
          </ul>
        </div>
      </div>
    ...
    <div class="content" v-show="!showA">
      ...
      <div class="table">
        <div class="bottom" v-for="item in lists3">
          <ul v-for="key in item.object" :class="[key.flag?'table-current':'']">
            <li @click="pour1(key)" :class="[key.num?'one':'border1_none']">
              <span :class="[key.color?key.color:'']">{{key.num}}</span>
            </li>
            <li @click="pour1(key)" :class="[key.num?'two':'border2_none']">{{key.odd}}</li>
            <li @click.self="pour1(key)" :class="[key.num?'three':'border3_none']">
              <I-Input :value="key.money" @on-blur="onblur(key,2)" @on-keydown="tab_now(key)" @on-change="onchange(key)" :maxlength='9' ref="input1" @on-focus="onfocus(key,2)" style="width: 45px"  v-if="key.num" v-model="key.money" @on-keyup="gogo(key)" @on-afterpaste="gogo(key)" size="small"></I-Input>
            </li>
          </ul>
        </div>
      </div>
    ...
    <div class="bottom_content" v-show="showA">
      <div class="top">
        <ul>
          <li class="one">类型</li>
          <li class="two">赔率</li>
        ...
        <div class="bottom" v-for="item in lists2">
          <ul v-for="key in item.object" :class="[key.flag?'table-current':'']">
            ...
            <li @click.self="pour_other(key)" :class="[key.num?'three':'border3_none']">
              <I-Input :value="key.money" @on-blur="onblur(key,3)" @on-keydown="tab_now(key)" @on-change="onchange(key)" :maxlength='9' ref="input_other" @on-focus="onfocus(key,3)" v-if="key.num" style="width: 45px" v-model="key.money" @on-keyup="gogo(key)" @on-afterpaste="gogo(key)" size="small"></I-Input>
            </li>
          </ul>
        </div>
      </div>
...
export default {
      ...
      lists2:[
        {
          name:'1',object:[
            {
              index:0,num:'1-10',flag:false,number:'4.5',money:''
            },
            {
              index:1,num:'大',flag:false,number:'1.93',money:''
            },
            {
              index:2,num:'小',flag:false,number:'1.93',money:''
            },
            {
              index:3,num:'单',flag:false,number:'1.93',money:''
            },
            {
              index:4,num:'双',flag:false,number:'1.93',money:''
            },
          ]
        },
        {
          name:'2',object:[
            {
              index:5,num:'11-20',flag:false,number:'4.5',money:''
            },
            {
              index:6,num:'大单',flag:false,number:'1.93',money:''
            },
            {
              index:7,num:'小单',flag:false,number:'1.93',money:''
            },
            {
              index:8,num:'大双',flag:false,number:'1.93',money:''
            },
            {
              index:9,num:'小双',flag:false,number:'1.93',money:''
            },
          ]
        },
        {
          name:'2',object:[
            {
              index:10,num:'21-30',flag:false,number:'4.5',money:''
            },
            {
              index:11,num:'合大',flag:false,number:'3.8',money:''
            },
            {
              index:12,num:'合小',flag:false,number:'3.8',money:''
            },
            {
              index:13,num:'合单',flag:false,number:'3.8',money:''
            },
            {
              index:14,num:'合双',flag:false,number:'3.8',money:''
            },
          ]
        },
        {
          name:'2',object:[
            {
              index:15,num:'31-40',flag:false,number:'4.5',money:''
            },
            {
              index:16,num:'尾大',flag:false,number:'1.93',money:''
            },
            {
              index:17,num:'尾小',flag:false,number:'1.93',money:''
            },
            {
              index:18,num:'家禽',flag:false,number:'1.93',money:''
            },
            {
              index:19,num:'野兽',flag:false,number:'1.93',money:''
            },
          ]
        },
        {
          name:'1',object:[
            {
              index:20,num:'1-10',flag:false,number:'4.5',money:''
            },
            {
              index:21,num:'红波',flag:false,number:'1.93',money:''
            },
            {
              index:22,num:'绿波',flag:false,number:'1.93',money:''
            },
            {
              index:23,num:'蓝波',flag:false,number:'1.93',money:''
            },
            {
              index:'',num:'',number:'',money:''
            },
          ]
        },
      ],
  ...
  watch: {
    // 如果路由有变化，会再次执行该方法
    '$route.query.page': 'fetchData',   // 只有这个页面初始化之后，这个监听事件才开始生效
  ...
  methods:{
    config_num: function (e) {
      ...
      if(this.showA){
        ...
        for (let i = 0; i < this.lists3.length; i++) {
          for (let j = 0; j < this.lists3[i].object.length; j++) {
            let input_name = Number(this.lists3[i].object[j].input_name);
            //合值
            var c = input_name%10+parseInt(input_name/10 % 10);
            if(e.index == 0){
              if(this.lists3[i].object[j].color == 'red'&& input_name > 24){
                this.lists3[i].object[j].flag = true;
                this.lists3[i].object[j].money = this.money;
              }
            }else if(e.index == 1){
              if(this.lists3[i].object[j].color == 'red'&& c%2 == 1){
                this.lists3[i].object[j].flag = true;
                this.lists3[i].object[j].money = this.money;
              }
            }else if(e.index == 2){
              if(this.lists3[i].object[j].color == 'red'&& input_name%2 == 0){
                this.lists3[i].object[j].flag = true;
                this.lists3[i].object[j].money = this.money;
              }
            }else if(e.index == 3){
              ...
              if(this.lists3[i].object[j].color == 'red'&& c%2 == 0){
                this.lists3[i].object[j].flag = true;
                this.lists3[i].object[j].money = this.money;
              }
            }else if(e.index == 5){
              ...
              if(this.lists3[i].object[j].color == 'green'&& input_name > 24){
                this.lists3[i].object[j].flag = true;
                this.lists3[i].object[j].money = this.money;
              }
            }else if(e.index == 7){
              if(this.lists3[i].object[j].color == 'green'&& c%2 == 1){
                this.lists3[i].object[j].flag = true;
                this.lists3[i].object[j].money = this.money;
              }
            }else if(e.index == 8){
              if(this.lists3[i].object[j].color == 'green'&& input_name%2 == 1){
                this.lists3[i].object[j].flag = true;
                this.lists3[i].object[j].money = this.money;
              }
            }else if(e.index == 9){
              if(this.lists3[i].object[j].color == 'green'&& input_name%2 == 0){
                this.lists3[i].object[j].flag = true;
                this.lists3[i].object[j].money = this.money;
              }
            }else if(e.index == 10){
              if(this.lists3[i].object[j].color == 'green'&& c%2 == 0){
                this.lists3[i].object[j].flag = true;
                this.lists3[i].object[j].money = this.money;
              }
            }else if(e.index == 11){
              ...
              if(this.lists3[i].object[j].color == 'blue'&& input_name > 24){
                this.lists3[i].object[j].flag = true;
                this.lists3[i].object[j].money = this.money;
              }
            }else if(e.index == 13){
              if(this.lists3[i].object[j].color == 'blue'&& c%2 == 1){
                this.lists3[i].object[j].flag = true;
                this.lists3[i].object[j].money = this.money;
              }
            }else if(e.index == 14){
              if(this.lists3[i].object[j].color == 'blue'&& input_name%2 == 0){
                this.lists3[i].object[j].flag = true;
                this.lists3[i].object[j].money = this.money;
              }
            }else if(e.index == 15){
              if(this.lists3[i].object[j].color == 'blue'&& input_name%2 == 1){
                this.lists3[i].object[j].flag = true;
                this.lists3[i].object[j].money = this.money;
              }
            }else if(e.index == 16){
              if(this.lists3[i].object[j].color == 'blue'&& c%2 == 0){
                this.lists3[i].object[j].flag = true;
                this.lists3[i].object[j].money = this.money;
              }
            }else if(e.index == 17){
        ...
        for (let i = 0; i < this.lists3.length; i++) {
          for (let j = 0; j < this.lists3[i].object.length; j++) {
            ...
            if(e.index == 22){//鼠
              for(let b=0;b<this.animal_config.mouse.length;b++){
                if(input_name == this.animal_config.mouse[b]){
                  this.lists3[i].object[j].flag = e.state;
                  if(e.state){
                    this.lists3[i].object[j].money = this.money;
                  }else{
                    this.lists3[i].object[j].money = '';
                  }
                }
              }
            }else if(e.index == 23){//牛
    ...
    go_to:function () {
      ...
      if(this.showA){
        ...
        for (let i = 0; i < this.lists3.length; i++) {
          for (let j = 0; j < this.lists3[i].object.length; j++) {
            let d = this.lists3[i].object[j].money + 'd';
            this.lists3[i].object[j].money = d.replace(/\D/g, "");
            kk += Number(this.lists3[i].object[j].money);
            if(this.lists3[i].object[j].flag){
              is_select = true
            }
          }
        }
...
//          document.querySelector('body').style.overflow='hidden';
          if(this.showA){
            ...
            this.$root.$emit('id-selected',this.lists3)
          }
        }else if (kk == 0) {
          this.$Modal.warning({
    ...
    computed_money: function(){
      ...
      for (let i = 0; i < this.lists3.length; i++) {
        for (let j = 0; j < this.lists3[i].object.length; j++) {
          ...
          if (this.lists3[i].object[j].flag) {
            this.lists3[i].object[j].money = this.money
          }else if(!this.lists3[i].object[j].flag){
            this.lists3[i].object[j].money = ''
          }
        }
      }
    ...
    fetchData: function(type){
      this.reset();
      if(this.$route.query.page != 'liuhecai'){
...
//          this.$root.$emit('get_closetime', res.data.Data.closetime);
    ...
    fect_data2: function(){
      ...
      api.getgameindex(this,body1,(res)=>{
        if(res.data.ErrorCode == 1){
          this.$root.$emit('loading',false);
          let back_data = res.data.Data.odds;
          back_data.sort(this.sortNumber);
          this.computed3(back_data);
        }
      })
    },
    ...
    computed1: function(data){
      ...
      for(let i = 1;i < this.lists2[0].object.length;i++){
        Object.assign(this.lists2[0].object[i],data[53+i]);
        let name = data[53+i].remark.slice(
          data[53+i].remark.search("#") + 1,
          data[53+i].remark.length
        );
        this.lists2[0].object[i].num = name;
      }
      for(let i = 1;i < this.lists2[1].object.length;i++){
        Object.assign(this.lists2[1].object[i],data[57+i]);
        let name = data[57+i].remark.slice(
          data[57+i].remark.search("#") + 1,
          data[57+i].remark.length
        );
        this.lists2[1].object[i].num = name;
      }
      for(let i = 1;i < this.lists2[2].object.length;i++){
        Object.assign(this.lists2[2].object[i],data[61+i]);
        let name = data[61+i].remark.slice(
          data[61+i].remark.search("#") + 1,
          data[61+i].remark.length
        );
        this.lists2[2].object[i].num = name;
      }
      for(let i = 1;i < this.lists2[3].object.length;i++){
        Object.assign(this.lists2[3].object[i],data[65+i]);
        let name = data[65+i].remark.slice(
          data[65+i].remark.search("#") + 1,
          data[65+i].remark.length
        );
        this.lists2[3].object[i].num = name;
      }
      for(let i = 1;i < this.lists2[4].object.length;i++){
        Object.assign(this.lists2[4].object[i],data[69+i]);
        let name = data[69+i].remark.slice(
          data[69+i].remark.search("#") + 1,
          data[69+i].remark.length
        );
        this.lists2[4].object[i].num = name;
      }
    },
    computed3: function (data) {
      this.$set(this.lists3,this.lists3);
      for(let i = 0;i < this.lists3[0].object.length;i++){
        Object.assign(this.lists3[0].object[i],data[i]);
        let name = data[i].remark.slice(
          data[i].remark.search("#") + 1,
          data[i].remark.length
        );
        this.lists3[0].object[i].num = name;
      }
      for(let i = 0;i < this.lists3[1].object.length;i++){
        Object.assign(this.lists3[1].object[i],data[13+i]);
        let name = data[13+i].remark.slice(
          data[13+i].remark.search("#") + 1,
          data[13+i].remark.length
        );
        this.lists3[1].object[i].num = name;
      }
      for(let i = 0;i < this.lists3[2].object.length;i++){
        Object.assign(this.lists3[2].object[i],data[26+i]);
        let name = data[26+i].remark.slice(
          data[26+i].remark.search("#") + 1,
          data[26+i].remark.length
        );
        this.lists3[2].object[i].num = name;
      }
      for(let i = 0;i < this.lists3[3].object.length;i++){
        if(i == 10){
          break;
        }else{
          Object.assign(this.lists3[3].object[i],data[39+i]);
          let name = data[39+i].remark.slice(
            data[39+i].remark.search("#") + 1,
            data[39+i].remark.length
          );
          this.lists3[3].object[i].num = name;
        }
      }
    },
    go_child: function(e){
    ...
    onblur: function (key,type) {
        if(type == 1){
          key.money = this.$refs.input[key.index].$refs.input.value;
        }else if(type == 2){
          key.money = this.$refs.input1[key.index].$refs.input.value;
        }else if(type == 3){
    ...
    pour: function(item) {
      ...
      onfocus: function(item,type){
        ...
        if(type == 1){
          ...
          this.$refs.input1[item.index].$refs.input.data_onoff = 'true';
          this.a = item.index;
          for(let i = 0;i < dom.length;i++){
              if(i != item.index+50) {
                  dom[i].data_onoff = 'false';
    ...
    pour1: function(item) {
        ...
        if(item.money && item.flag == true){
          item.flag = false;
          item.money = '';
          this.$refs.input1[item.index].blur();
          this.$refs.input1[item.index].$refs.input.value = '';
        }else if(item.money == '' && item.flag == false){
          this.$refs.input1[item.index].focus();
        }else if (item.flag == true && item.money == ''){
          item.flag = false;
          this.$refs.input1[item.index].$refs.input.value = '';
        }else if (item.flag == true){
...
<style lang="scss" src="../../../assets/css/six_one.scss" scoped></style>
