            ...
            <li v-for="(key,k) in item.object" :class="key.flag?'styleclect':''" class='sonli'>
              <ul class="clearfix sonul">
                ...
                <li class="three" @click.self="myclick(key)">
                 <I-Input @on-keydown="tab_now(key)" @on-change="onchange(key)" :value="key.money" @on-blur="onblur(key)" :maxlength="9" class="inp" ref="myfocus" style="width: 50px" @on-focus="onfocus(key)" v-model="key.money" size="small" @on-keyup="gogo(key)" @on-afterpaste="gogo(key)"></I-Input>
                </li>
              </ul>
            </li>
          </ul>
