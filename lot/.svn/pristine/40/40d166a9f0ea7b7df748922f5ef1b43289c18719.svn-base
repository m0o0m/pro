...
use common\models\NoticebackModel as selfModel;
use common\models\AgentModel;
use yii\helpers\ArrayHelper;

class NoticebackController extends Controller {
    ...
    public function actionAgent() {
        $request = Yii::$app->request->post();
        $result = self::agent($request); // 切换代理类型
    ...
    public function trans($data, $showData) {
        ...
        foreach ($data as $k => &$v) {
            ...
            $v['lineTxt'] = key_exists($v['line_id'], $showData['lines']) ? $showData['lines'][$v['line_id']]['line_name'] : $v['line_id'];
    ...
    public function addSubmit($request) {
        ...
        $type = isset($request['type']) ? trim($request['type']) : 1;
        ...
        $this->batchDelRedis('after', '_backend_notice' );// 后台线路公告信息的key
  ...
  public static function getAgentByType($agent_type = '') {
        if(!$agent_type){
            return [];
        }
        ...
        return self::getAgentItems('my_user_' . $agent_type, $where);
    }
