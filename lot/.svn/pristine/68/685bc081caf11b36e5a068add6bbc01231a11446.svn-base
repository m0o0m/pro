    ...
    <div class="bottom_content">
      ...
      <div class="table">
        <div class="bottom" v-for="item in lists">
          <ul v-for="key in item.object" :class="[key.flag?'styleclect':'']">
            <li @click="myclick(key)" class="one">{{key.num}}</li>
            <li @click="myclick(key)" class="one">{{key.odd}}</li>
            <!-- <li :class="[key.number?'three':'border3_none']"><input v-if="key.number" type="text" ref="input_other" v-model="key.item"/></li> -->
            <li class="two" @click.self="myclick(key)">
              <I-Input :value="key.money" @on-blur="onblur(key)" @on-focus="onfocus(key)" @on-keydown="tab_now(key)" @on-change="onchange(key)" :maxlength='9' ref="myfocus" style="width: 50px" class="mr" size="small"  v-model="key.money" @on-keyup="gogo(key)" @on-afterpaste="gogo(key)"></I-Input>
            </li>
            <li class="four" @click="myclick(key)">
