    ...
    <div class="r_box">
      <div class="table">
        ...
        <ul v-for="(key,index) in lists" class="from">
          <li @click.stop="myclick(key,index)" class="li1">
            <ul class="ul1 clearfix">
              ...
              <li>{{key.bet}}</li>
              <li>{{key.valid_bet}}</li>
              <li class="red" v-if="key.win!=0">{{key.win}}</li>
              <li v-else>未中奖</li>
            </ul>
          </li>
          <li class="li2" ref="myref">
            <ul @click="tiaozhuan(item)" v-for="(item,i) in key.list" class="ul2 clearfix">
              <li></li>
        ...
        <ul class="ul3 clearfix" v-if="this.lists.length">
          ...
          <li :class="{'red':totals.win!=0}">{{totals.win}}</li>
        </ul>
...
//            Object.assign(this.lists[i], res.data.Data[i]);
            // if (this.lists[i].win == 0) {
            //   this.lists[i].win = "未中奖";
            // }
            // for (var l = 0; l < this.lists[i].list.length; l++) {
            //   if (this.lists[i].list[l].win == 0) {
            //     this.lists[i].list[l].win = "未中奖";
            //   }
            // }
//          }
        }
