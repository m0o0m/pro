      ...
      <ul v-for="item in eightlist">
        ...
        <li v-for="(key,index) in item.object" :class="[key.flag?'styleclect':'']">
          ...
          <span class="three" @click.self="myclick(key,index)">
             <I-Input :value="key.money" @on-blur="onblur(key)" @on-keydown="tab_now(key)" @on-change="onchange(key)" :maxlength="9" v-if="key.num" ref="myfocus" size="small" style="width: 60px;" v-model="key.money" @on-focus="onfocus(key)" @on-keyup="gogo(key)" @on-afterpaste="gogo(key)"></I-Input>
           </span>
        </li>
      </ul>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      go_to() {
        ...
        var kk = 0;
        var is_select = false;
        for (let i = 0; i < this.eightlist.length; i++) {
          for (let j = 0; j < this.eightlist[i].object.length; j++) {
            let b = this.eightlist[i].object[j].money + 'b';
            this.eightlist[i].object[j].money = b.replace(/\D/g, "");
            kk += Number(this.eightlist[i].object[j].money);
            if(this.eightlist[i].object[j].flag){
              is_select = true
            }
          }
        }
...
//            document.querySelector('body').style.overflow = 'hidden';
            this.$root.$emit('id-selected', this.eightlist);
      ...
      reset: function () {
        for (let i = 0; i < this.eightlist.length; i++) {
          for (let j = 0; j < this.eightlist[i].object.length; j++) {
            this.eightlist[i].object[j].money = '';
            this.eightlist[i].object[j].flag = '';
          }
        }
        this.money = '';
      ...
      computed_money: function () {
        for (let i = 0; i < this.eightlist.length; i++) {
          for (let j = 0; j < this.eightlist[i].object.length; j++) {
            //添加金额参数入对象
            if (this.eightlist[i].object[j].flag) {
              this.eightlist[i].object[j].money = this.money
            }else if(!this.eightlist[i].object[j].flag){
              this.eightlist[i].object[j].money = ''
            }
          }
        }
