      ...
      <ul v-for="item in sixlist">
        ...
        <li v-for="(key,index) in item.object" :class="[key.flag?'styleclect':'']">
          ...
          <span class="three" @click.self="myclick(key,index)">
             <I-Input :maxlength="9" v-if="key.num" :value="key.money" @on-blur="onblur(key)" ref="myfocus" size="small" style="width: 60px;" v-model="key.money"
  ...
  export default {
    ...
    methods: {
      ...
      go_to() {
        ...
        var kk = 0;
        var is_select = false;
        for (let i = 0; i < this.sixlist.length; i++) {
          for (let j = 0; j < this.sixlist[i].object.length; j++) {
            if (this.sixlist[i].object[j].money != undefined) {
              let b = this.sixlist[i].object[j].money + 'b';
              this.sixlist[i].object[j].money = b.replace(/\D/g, "");
              kk += Number(this.sixlist[i].object[j].money);
              if(this.sixlist[i].object[j].flag){
                is_select = true
              }
            }
          }
...
//            document.querySelector('body').style.overflow = 'hidden';
            this.$root.$emit('id-selected', this.sixlist);
      ...
      reset: function () {
        for (let i = 0; i < this.sixlist.length; i++) {
          for (let j = 0; j < this.sixlist[i].object.length; j++) {
            this.sixlist[i].object[j].money = '';
            this.sixlist[i].object[j].flag = '';
          }
        }
        this.money = '';
      ...
      computed_money: function () {
        for (let i = 0; i < this.sixlist.length; i++) {
          for (let j = 0; j < this.sixlist[i].object.length; j++) {
            //添加金额参数入对象
            if (this.sixlist[i].object[j].flag) {
              this.sixlist[i].object[j].money = this.money
            } else if (!this.sixlist[i].object[j].flag) {
              this.sixlist[i].object[j].money = ''
            }
          }
        }
