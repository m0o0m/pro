...
use \libraries\balance\LiuhecaiBalance;

class JsliuhecaiBalance {
    const WIN   = 2;
    const FAIL  = 3;
    const DRAW  = 4;
    const ONE   = 5;
    const TWO   = 6;
    static $type = "jsliuhecai";
    static $count = [];         //总用户数
    static $info = [];          //用户输赢
    static $bets = [];          //总注单
    ...
    static $close       = 0;    //封盘时间
    static $next_sec    = 120;  //下期间隔

    public static function Balance_bet($result, $info){
        return LiuhecaiBalance::Balance_bet($result, $info);
    }
