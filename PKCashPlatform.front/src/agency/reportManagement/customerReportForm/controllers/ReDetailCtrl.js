angular.module('app.ReportForm').controller('ReDetailCtrl',
    function(httpSvc,popupSvc,$scope,reportformService,APP_CONFIG,$rootScope,$stateParams,$state,$timeout){
    //获取参数
        var postData = {
                site_index_id:$stateParams.site_index_id,
                time_zone:$stateParams.time_zone,
    ...
    //获取详情列表
    reportformService.reportquery(postData).then(function (res) {
        $scope.data = res;
    });
    ...
    $scope.aa = function (event) {
        ...
        if (flag==2){
            ...
            reportformService.hareholdersStatement(id).then(function (res) {
                ...
                $(".bb").click(function(event){
                    ...
                    if(flag==2) {
                        ...
                        reportformService.generalGenerationReport(iD).then(function (res) {
                            ...
                            $(".cc").click(function(event){
                                ...
                                if(flag==2) {
                                    var id = $(target).attr('myid');
                                    reportformService.proxyReport(id).then(function (res) {
