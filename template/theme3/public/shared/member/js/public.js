...
function pageList(metalist, links) {
    ...
    if(current_page==1){
        htmldata+= '<span class="iconfont icon-back page-disable"></span>';
    }else{
        htmldata+= '<span class="iconfont icon-back"></span>';
    }
    ...
    if(current_page==totalPage){
        htmldata += '<span class="iconfont icon-forward page-disable"></span>';
    }else{
...
function GetPageData(FuncName, metadata) {
    ...
    $('.one-foot .icon-back').click(function () {
    ...
    $('.one-foot .icon-forward').click(function () {
        var page = $('.xla_k option:selected').val();
        var getpage = Number(page) + 1;
        console.log(getpage)
        if (page > (metadata - 1)) {
