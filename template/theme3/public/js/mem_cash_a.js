...
$(function () {
    ...
    $('#myFORM').find('select[name^=pwd]').focus(function () {
        ...
        validator.resetForm();
        $("#bank_county_form").html('<option value="">--请选择城市--</option>')
    });
    ...
    $.validator.addMethod('ajaxCheckData', function (value, element) {
        ...
        var data = {
            'r_user_form': {'ajax': 'CheckAgentUser', 'user': value},
            'yzm_form': {'ajax': 'CheckCode', 'code': value},
        };
    ...
    /**/
    $.validator.addMethod('_CheckIDCard', function (value, element) {
        if ($('select[name=card]').val() == '')
            $('select[name=card]').focus();
        return ($('select[name=card]').val() != '') ? true : false;
    }, '请选取身份证或护照选项！');
    ...
    var validator = $("#myFORM").validate({
        ...
        'messages': {
            ...
            'password_form': {
                ...
                'minlength': $._BuildPrompt('password_form', '✖ 请输入6-11个字元的密码!!', {'width': 170}),
            ...
            'yzm_form': {
                'required': $._BuildPrompt('yzm_form', '✖ 请输入验证码!!', {'width': 120}),
        ...
        'submitHandler': function () {
            for (var i in query) {
                if (!query[i]['status']) {
                    console.log(query[i]['status'])
                    return false;
                }
            }
            if (confirm("是否确定写入?")) {
                $('form input:submit').attr('disabled', 'disabled');
                var code = $('input[name=yzm_form]').val();
                $.ajax({
                        url: '/agency/register',
                        type: 'post',
                        headers: {
                        ...
                        data: JSON.stringify({
                            chineseNickname: $('input[name=chinese_nick_name]').val(),
                            englishNickName: $('input[name=english_nick_name]').val(),
                            agencyAccount: $('input[name=r_user_form]').val(),
                            password: $('input[name=password_form]').val(),
                            confirmPassword: $('input[name=passwd_form]').val(),
                            code: code,
                            userName: $('input[name=user_name]').val(),
                            qq: $('input[name=qq]').val(),
                            email: $('input[name=email]').val(),
                            phone: $('input[name=mobile]').val(),
                            otherMethod: $('input[name=other_method]').val(),
                            promoteWebsite: $('input[name=promote_website]').val(),
                            bankId: $("#bank_type_form option:selected").val(),
                            backAccount: $("input[name=back_account]").val(),
                            province: $("#bank_province_form option:selected").val(),
                            zone: $("#bank_county_form option:selected").val(),
    ...
    $('#myFORM input[name=email]').rules('add', {
        'required': false,
        'messages': {
            'required': $._BuildPrompt('email', '✖ 请输入有效的邮箱！', {
    ...
    $('#myFORM input[name=email]').rules('add', {
        'email': false,
        'messages': {
            'email': $._BuildPrompt('email', '✖ E-mail格式不正确!!', {
                'positionType': 'bottomLeft',
                'width': 120,
                'left': -50,
                'top': 10
