    ...
    if ( getCookie('loginBack') ) {
        ...
        for (var k in loginData.TBalance) {
            ...
            if (v["name"] == '账户总余额' || v["name"] == '账户余额') {
                obalanceStr += '<div class="ele-obalance"><span>' + v["name"] + '：</span><strong>' + v["balance"] + '</strong></div>';
            }else{
                obalanceStr += '<div class="ele-obalance"><span>' + v["name"] + '余额：</span><strong>' + v["balance"] + '</strong></div>';
            }
        }
        ...
        $.ajax({
            ...
            success: function (msg) {
                if (!msg['code']) {
                    ...
                    if(sdata.birthday != ''){
                        birthdayObj.val(date("Y-m-d h:i:s",sdata.birthday));
                    }else{
                ...
                if (!msg['code']) {
                    ...
                    if (sdata) {
                        for (var k in sdata) {
                            var v = sdata[k];
                            htmlStr += '<li>'+
                                '<p class="fl one-bottom-hl">'+date('Y-m-d', v.create_time)+'<br/>'+date('H:i:s', v.create_time)
                                '</p>'+
    ...
    $("div.my-dialog").on('click', '.dialog-submit', function(){
        if(getCookie('loginBack')){
            ...
            if ($titleName == 'one-info-dialog') {
                $birthday_num = $formObj.find("input[name='birthday_num']").val();
                $email_num = $formObj.find("input[name='email_num']").val();
                $phone_num = $formObj.find("input[name='phone_num']").val();
                $qq_num = $formObj.find("input[name='qq_num']").val();
                $wechat = $formObj.find("input[name='wechat']").val();
                $card = $formObj.find("input[name='card']").val();
                $local_code = '+86';
                $remark = $formObj.find("input[name='remark']").val();
                if($birthday_num != ''){
                    if($birthday_num.length >10){
                        alert('会员生日格式不正确');return;
                    }
                }
                if ($email_num != '') {
                ...
                var $success = function(msg){
                    if (msg) {
                        ...
                        alert('会员资料修改成功！');
                        $("div.one-right-bottom .right-b-l .icon-xiaoxi").siblings('input').val(data['email_num']);
                        $('.my-dialog').hide();
                        return
                    }
                };
