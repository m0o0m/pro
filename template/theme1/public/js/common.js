...
function MemberMsg() {
    try {
        ...
        $.ajax({
            type: 'POST',
            url: '?module2=MemberMsg&method=getMsg',
            error: function(msg) {
                alert('error');
            },
...
function HotNewsHistory() {
     ...
     window.open('/cl/?module2=MFunction&method=ShowHotNewsHistory', 'HotNewsHistory', features);
}
...
JackPotRule = function() {
    window.open('?module2=MAdvertis&method=JackPotRule', 'popup','width=658,height=280,scrollbars=no,resizable=no, toolbar=no,directories=no,location=no,menubar=no, status=no,left=0,top=0');
};

//取得賽事數量
var _TmpGameQuantity = {};
function GetGameQuantity(color){
   $.getJSON('/cl/?module2=MCountGameNums&method=CountGames',{ "noCache": Math.random()}, function(data){
...
function GetFavorQuantity(color) {
    $.getJSON('/cl/?module2=MCountGameNums&method=CountFavoGames', {"noCache": Math.random()}, function(data) {
...
function gameRule(GAME_TYPE) {
    window.open('/cl/?module2=MRule&method=ruleDescrip&args=' + GAME_TYPE, 'AllRule', 'width=1024,height=640,status=no,scrollbars=yes');
}
...
/**
 * 開啟會員中心頁面
 * @param {String} mo module2
 * @param {String} me method
 */
f_com.MGetPager = function(mo, me) {
    window.open("/cl/?module2=" + mo + "&other=" + me, "MACENTER", "top=50,left=50,width=1020,height=610,status=no,scrollbars=yes,resizable=no").focus();
};
...
f_com.BBinGetPager = function(mo) {
    window.open("/cl/?module2=" + mo, "bb", "top=50,left=50,width=1000,height=600,status=no,scrollbars=no,resizable=yes").focus();
};
...
f_com.BBinEnterPager = function(mo,me,ty) {
    //視窗寬度ub連覽器判斷
    if (!ty){
        window.open("/cl/?module2=" + mo +"&PageItem=" + me , "BBinPage", "top=50,left=50,width=1000,height=600,status=no,scrollbars=no,resizable=yes").focus();
    }else{
        window.open("/cl/?module2=" + mo +"&PageItem=" + me+"&type=" + ty , "BBinPage", "top=50,left=50,width=1000,height=600,status=no,scrollbars=no,resizable=yes").focus();
    }
};
/*開啟bbin資訊中心-指定頁面並無左側選單*/
f_com.BBinOnlyPager = function(mo,me,ty)  {
    if (!ty){
        window.open("/cl/?module2=" + mo +"&OnlyPage=Y&PageItem=" + me , "BBinPage", "top=50,left=50,width=770,height=700,status=no,scrollbars=no,resizable=yes").focus();
        return;
    }

    window.open("/cl/?module2=" + mo +"&PageItem=" + me+"&OnlyPage=Y&type=" + ty , "BBinPage", "top=50,left=50,width=770,height=700,status=no,scrollbars=no,resizable=yes").focus();
}
/*event活動開啟*/
