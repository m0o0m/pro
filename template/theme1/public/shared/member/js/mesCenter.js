    ...
    if (getCookie('loginBack')) {
        $.ajax({
            ...
            success: function (data) {
                // $('.o2 dl ul').html()
                console.log(data)
                var htmldata = ' <ul class="ul1 dd clearfix  my-theme">\n' +
                    '                                                    <li>状态</li>\n' +
                    '                                                    <li>日期</li>\n' +
                    '                                                    <li>标题</li>\n' +
                    '                                                    <li>操作</li>\n' +
                ...
                if (data != null) {
                    for (var i = 0; i < data.length; i++) {
                        ...
                        htmldata += '<ul class="ul1f dd clearfix"><div class="show-msg-block"  value="' + data[i].id + '" stat="' + data[i].state + '"> ';
                        if (data[i].state == 2) {
                        ...
                        htmldata += readli + '<li>' + mestime[0] + mestime[1] + '</li> ' +
                            '<li class="pk_title">' + data[i].title + '  </li></div>' +
                ...
                $('.del').click(function () {
                    var id = $(this).parent().parent().find('.pk_title').attr('value');
...
$('.nav2 .show-notice-list').click(function () {
    ...
    GetNoticeData(th, Number(id))
})

function GetNoticeData(th, id) {
    var stat = th.attr('stat');
    $.ajax({
        url: '/ajax/noticeList',
        data: {id: id},
        headers: {
        ...
        success: function (data) {
            ...
            $('#notice-show').children('ul').click(function(){
                console.log(123);
                $('.myd-modal').show();
            })

            $('.myd-modal-3').on('click','a',function(){
                $('.myd-modal').hide();
            })
        }
    })
