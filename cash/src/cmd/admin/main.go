...
func main() {
	...
	if ENVIRONMENT == "development" {
		...
		configFile := filepath.Join(filepath.Dir(path), "etc/admin.yaml")
		//加载config
		...
		cfg, err = config.ParseFromMongoDB(mongo_url, "/conf/p_config/p_admin.yaml")
		if err != nil {
			log.Fatalf("parse config file error:%v\n", err.Error())
			return
		}
	...
	//全局初始化
	global.InitLog(cfg, "admin")
	defer func() {
		time.Sleep(1 * time.Second)
		global.GlobalLogger.Close()
	}()
	//配置文件全局变量
	site.TotalConfig = cfg
	//初始化数据库
	err = global.InitMysql(cfg.Mysqls)
	if err != nil {
		//数据库连接错误
		log.Fatalf("InitDb error:%v\n", err.Error())
		return
	}
	...
	//启动web服务
	app, err := framework.NewAdmin(cfg)
	if err != nil {
		//错误日志
		log.Fatalf("NewApp error:%v\n", err.Error())
		return
	}
...
//定时任务
func CronAdmin(cfg config.RedisConfig) {
...
func Runs() {
	//弹出key
	keys, err := global.GetRedis().LPop("admin_login").Result()
	if err == redis.Nil {
		return
	} else if err != nil {
	...
	var s global.AdminRedisStruct
	json.Unmarshal([]byte(result), &s)

	//过期
	if s.ExpirTime < time.Now().Unix() {
		//根据id删除login_token
		err = UpAdminLoginStatus(s.Id)
		if err != nil {
			global.GlobalLogger.Error("error:%v\n", err.Error())
			return
		}
		...
		//不过期
		err = global.GetRedis().RPush("admin_login", keys).Err()
		if err != nil {
			global.GlobalLogger.Error("error:%v\n", err.Error())
			return
		}
...
//更新
func UpAdminLoginStatus(adminId int64) (err error) {
	var admin_bean = new(function.AdminBean)
	_, err = admin_bean.UpAdminLoginStatus(adminId)
	return
}
