...
func (captcha *CaptchaImage) DrawTextNoise(complex int) error {
	...
	rawFontSize := float64(captcha.height) / (1 + float64(r.Intn(7))/float64(20))
...
func (captcha *CaptchaImage) DrawText(text string) error {
	...
	for i, s := range text {
		...
		fontSize := float64(captcha.height) / (1 + float64(2)/float64(9))
		...
		y := captcha.height/2 + int(fontSize/2) - 4
