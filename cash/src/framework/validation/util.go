...
func gerErrorCode(f reflect.StructField) int64 {
	var defaurCode int64 = 10000
	tag := f.Tag.Get(ValidTag)
	hasCode := false
	if len(tag) > 0 {
		fs := strings.Split(tag, ";")
		for _, vfunc := range fs {
			if strings.Contains(vfunc, "ErrorCode(") {
				index := strings.Index(vfunc, "(")
				if index == -1 {
					hasCode = false
					continue
				}
				end := strings.LastIndex(vfunc, ")")
				if end < index {
					hasCode = false
					continue
				}
				code, err := strconv.ParseInt(vfunc[index+1:end], 10, 64)
				if err != nil {
					hasCode = false
					continue
				}
