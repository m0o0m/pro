...
//Qq Test that the obj is format of starting in 10000.
...
func (v *Validation) Valid(obj interface{}) (b bool, err error) {
	...
	v.ErrorCode = make(map[string]int64)
	switch {
	case isStruct(objT):
	...
	for i := 0; i < objT.NumField(); i++ {
		...
		v.ErrorCode[objT.Field(i).Name] = gerErrorCode(objT.Field(i))
		var vfs []ValidFunc
		//取出需要传进去校验的数据
