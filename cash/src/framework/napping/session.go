...
func (s *Session) Send(r *Request) (response *Response, err error) {
	...
	*/
	...
	if resp.Header.Get("Content-Encoding") == "gzip" {
		r.body, err = _DumpGZIP(resp.Body)
		if err != nil {
			return nil, err
		}
	} else {
	...
	//如果 匹配不到
	if string(r.body) != "" {
		//config.Log.Logf(logger.DEBUG, "Get data: %s", string(r.body))
		...
		if resp.StatusCode >= 400 && r.Error != nil {
			//错误只以json返回
			err = json.Unmarshal(r.body, r.Error) // Should we ignore unmarshall error?
		}
	}
	rsp := Response(*r)
	response = &rsp
...
func _DumpGZIP(r io.Reader) ([]byte, error) {
...
// Send constructs and sends an HTTP request.
func (s *Session) Send2(r *Request) (body string, err error) {
	r.Method = strings.ToUpper(r.Method)
	//验证url
	u, err := url.Parse(r.Url)
	if err != nil {
...
// Post sends a POST request. 以form方式提交不支持文件
func (s *Session) Post2(url string, p *Params, payload, result, errMsg interface{}) (string, error) {
	r := Request{
		Method:  "POST",
		Url:     url,
		Params:  p,
...
// Put sends a PUT request.
func (s *Session) Put2(url string, payload, p *Params, result, errMsg interface{}) (string, error) {
	r := Request{
		Method:  "PUT",
		Url:     url,
		Params:  p,
