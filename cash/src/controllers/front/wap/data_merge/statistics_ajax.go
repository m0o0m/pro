...
func (*StatisticsAjax) GetWeek(num, state int) (days []string) {
	var day string
	if state == 1 { //本周
		for i := 1; i <= 7; i++ {
			day = time.Now().AddDate(0, 0, i-num).Format("2006-01-02")
			days = append(days, day)
		}
	} else if state == 2 { //上周
		for j := 7; j >= 1; j-- {
			day = time.Now().AddDate(0, 0, 1-j-num).Format("2006-01-02")
			days = append(days, day)
		}
	}
	return
