...
//取款管理
type WapDrawBean struct {
}

//取款(没有给稽核日志表添加数据)
func (*WapDrawBean) Withdrawal(ctx echo.Context) error {
	...
	//err = wapMemberRedisSet(redisKey, b, redisKey)
	//if err != nil {
	//	global.GlobalLogger.Error("error:%s", err.Error())
	//	return ctx.JSON(500, global.ReplyError(60000, ctx))
	//}
	fmt.Printf("showoutdata %+v\n", sdata)
