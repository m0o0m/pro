...
func (*SignController) Login(ctx echo.Context) error {
	...
	log.Device = device
	log.Account = info.Account
	log.LoginIp = ctx.RealIP()
	log.SiteId = info.SiteId
	log.SiteIndexId = info.SiteIndexId
	log.LoginRole = 1
	...
	s := fmt.Sprintf(system+"member%s", time.Now().UnixNano()+info.Id)
	//生成tokenkey
...
func (*SignController) Register(ctx echo.Context) error {
	...
	flag, err = memberBean.CheckIsExist(register.Account, siteinfo.SiteId)
	if err != nil {
		global.GlobalLogger.Error("error:%s", err.Error())
		return ctx.JSON(500, global.ReplyError(60000, ctx))
	}
...
func GetSiteInfo(ctx echo.Context) (info schema.SiteDomain, flag bool, err error) {
	...
	var domain string
	if hs := strings.Split(host, ":"); len(hs) == 2 {
		domain = hs[0]
	} else {
		domain = host
	}
	info, flag, err = siteDomainBean.GetSiteByDomain(domain)
	return
}
...
//获取登录的时候存储的redis值
func (*SignController) getMemberRedis(token string) (string, error) {
